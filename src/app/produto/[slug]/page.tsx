...
function DetailStat({ label, value }: { label: string; value: string }) {
  return (
    <div className="flex items-start justify-between gap-6 border-t border-border py-4 first:border-t-0 first:pt-0 last:pb-0 sm:py-5">
      <p className="text-[0.72rem] uppercase tracking-[0.26em] text-muted">{label}</p>
      <p className="text-right text-[0.98rem] font-semibold text-foreground">{value}</p>
    </div>
  );
}
...
function RelatedProductCard({ product }: { product: Product }) {
  return (
    <Link href={`/produto/${product.slug}`} className="group block">
      <article className="flex h-full flex-col rounded-3xl border border-border bg-surface px-4 py-4 transition-all duration-300 hover:-translate-y-1 hover:shadow-[0_18px_40px_rgba(48,20,31,0.08)] dark:hover:shadow-[0_18px_40px_rgba(0,0,0,0.28)]">
        <div className="flex flex-1 items-center justify-center rounded-2xl border border-border bg-background p-4">
          <img
        ...
        <div className="mt-4 text-center">
          <h3 className="font-serif text-[0.92rem] uppercase tracking-[0.08em] text-foreground transition-colors group-hover:text-copper">
            {product.name}
          </h3>
          <p className="mt-1 text-sm font-semibold text-foreground">{formatCurrency(product.price)}</p>
        </div>
      </article>
    </Link>
  );
...
function ProdutoContent({
  ...
  return (
    <div className="min-h-screen bg-background px-4 pb-16 pt-6 text-foreground sm:px-6 lg:px-8 lg:pt-8">
      <ToastContainer />

      <div className="mx-auto max-w-7xl">
        <nav className="mb-7 flex items-center gap-2 overflow-x-auto whitespace-nowrap text-[0.72rem] text-muted sm:mb-10">
          <Link href="/" className="transition-colors hover:text-foreground">
            Home
          </Link>
          <ChevronRight size={14} className="shrink-0 text-muted" />
          <Link href={`/${product.category}`} className="transition-colors hover:text-foreground">
            {categoryLabel}
          </Link>
          <ChevronRight size={14} className="shrink-0 text-muted" />
          <span className="transition-colors hover:text-foreground">{breadcrumbLabel}</span>
          <ChevronRight size={14} className="shrink-0 text-muted" />
        ...
        <div className="grid gap-12 lg:grid-cols-[minmax(0,1.08fr)_minmax(360px,0.92fr)] xl:gap-16">
          <section className="grid gap-5 md:grid-cols-[124px_minmax(0,1fr)] md:items-start md:gap-8">
            <div className="hidden md:flex flex-col gap-5 pt-1">
              {galleryImages.map((image, index) => (
                <button
                  ...
                  className={cn(
                    "flex h-32 w-32 items-center justify-center overflow-hidden rounded-2xl border bg-surface p-2.5 transition-all duration-300",
                    activeImage === index
                      ? "border-foreground shadow-[0_12px_28px_rgba(48,20,31,0.08)] dark:shadow-[0_12px_28px_rgba(0,0,0,0.28)]"
            ...
            <div className="order-1 overflow-hidden rounded-[2.75rem] border border-border bg-surface p-5 sm:p-7 md:order-0 md:min-h-132">
            ...
            <div className="flex gap-4 overflow-x-auto pb-1 md:hidden">
              {galleryImages.map((image, index) => (
                <button
                  ...
                  onClick={() => setActiveImage(index)}
                  className={cn(
                    "h-24 w-24 shrink-0 overflow-hidden rounded-3xl border bg-surface p-2.5 transition-all",
          ...
          <section className="flex flex-col pt-2">
            <div className="flex flex-wrap gap-2">
              {product.tags.slice(0, 3).map((tag) => (
                <span
                  key={tag}
                  className="rounded-full bg-surface-2 px-3 py-1.5 text-[0.72rem] font-medium text-muted"
                >
                  {tag}
                </span>
              ))}
            ...
            <h1 className="mt-4 font-serif text-[2.45rem] uppercase leading-[0.95] tracking-[0.02em] text-foreground sm:text-[3.1rem] xl:text-[3.65rem]">
              {product.name}
            </h1>

            <p className="mt-5 max-w-xl text-[0.98rem] leading-7 text-muted">
              {product.description}
            </p>

            <div className="mt-6">
              <div className="border-b border-border pb-4"></div>
              {detailStats.map((stat) => (
                <DetailStat key={stat.label} label={stat.label} value={stat.value} />
              ))}
            </div>

            <div className="mt-6 inline-flex w-fit items-center gap-3 rounded-[1.25rem] bg-copper px-4 py-3 text-cream shadow-[0_16px_32px_rgba(48,20,31,0.18)]">
            ...
            <div className="mt-6 flex flex-col gap-4 border-t border-border pt-5 sm:flex-row sm:items-center">
              <div className="inline-flex h-12 items-center rounded-full bg-surface-2 px-1.5">
                <button
                  ...
                  disabled={quantity <= 1}
                  className="inline-flex h-10 w-10 items-center justify-center rounded-2xl text-foreground transition-colors hover:bg-black/5 dark:hover:bg-white/10 disabled:cursor-not-allowed disabled:opacity-40"
                  aria-label="Diminuir quantidade"
                >
                  <Minus size={18} />
                </button>
                <span className="min-w-10 px-2 text-center text-sm font-medium text-foreground">{quantity}</span>
                <button
                  type="button"
        ...
        <section className="mt-16 sm:mt-24">
          <div className="mx-auto max-w-3xl text-center">
            <h2 className="font-serif text-[1.45rem] uppercase tracking-[0.24em] text-foreground sm:text-[1.85rem]">
          ...
          <div className="mt-10 flex justify-center">
            <Link
              ...
              className="inline-flex h-11 items-center justify-center rounded-full border border-border bg-surface px-8 text-sm capitalize text-foreground transition-colors hover:bg-black/5 dark:hover:bg-white/5"
            >
              ver tudo
            </Link>
          </div>
